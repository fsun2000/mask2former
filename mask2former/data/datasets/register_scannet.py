...
def register_all_scannet_recursive(root):
    meta = _get_scannet_meta()
    for mode, dirname in [("train", "training"), ("val", "validation")]:
        name = f"scannet_sem_seg_{mode}"
        DatasetCatalog.register(name, lambda mode=mode: get_scannet_dicts(mode))
...
def get_scannet_dicts(mode): 
    root_dir = '/project/fsun/data/scannet'
...
#         mode = 'train'
    ...
    with open(os.path.join(root_dir, fragment_foldername, 'fragments_{}.pkl'.format(mode)), 'rb') as f:
        ...
        if mode == 'train':
            subsample_factor = subsample_every
        elif mode == 'val':
            subsample_factor = val_subsample_every

        if subsample_factor > 1:
            subsample_idx = np.arange(0, len(metas), subsample_factor)
...
register_all_scannet_recursive(_root)
